...
traceVarProcess = function(response,variablesHahMap){
	...
	if (Object.keys(variables).length > 0){
		...
		for (var name in variables) {
			...
			var input1 = '<input type="text" disabled="true" size="17" value="' + name+ '" style="margin: 0px 6px 0px 0px;font-weight: bold;">';
...
//===============================================================================
showInfoMsg = function(infoMsg_type, infoMsg_content)
{
	Swal.fire({
		  title: 'The following elements were modified:',
		  text: infoMsg_content,
		  icon: infoMsg_type,
...
//===============================================================================
showInfoMsg2 = function(infoMsg_type, infoMsg_content)
{
...
responseProcess = function(response,editor,graph){
	...
	var infoMsg_content = "";var infoMsg_type = "";
	...
	if (listChanges.traceID != null){
		...
		//DONOT use if(listChanges.infoMsg !== "undefined") instead use hasOwnProperty
		if(listChanges.hasOwnProperty("infoMsg")){
			infoMsg_type = listChanges.infoMsg[0];
			infoMsg_content = listChanges.infoMsg[1];
			
			showInfoMsg(infoMsg_type,infoMsg_content);
		}
		
		//================================================[MSGs]==========================================================
		if (listChanges.msg1 != null){
			//TODO: only one msg
			var srcMsgID = editor.getCapsuleIDfromHashMap(JSON.stringify(listChanges.msg1[0]).replace(/\"/g, ''));
			var trgMsgID = editor.getCapsuleIDfromHashMap(JSON.stringify(listChanges.msg1[1]).replace(/\"/g, ''));
		...
		if (listChanges.msg2 != null){
			//TODO: only one msg
			var srcMsgID = editor.getCapsuleIDfromHashMap(JSON.stringify(listChanges.msg2[0]).replace(/\"/g, ''));
			var trgMsgID = editor.getCapsuleIDfromHashMap(JSON.stringify(listChanges.msg2[1]).replace(/\"/g, ''));
		...
		if (listChanges.msg3 != null){
			//TODO: only one msg
			var srcMsgID = editor.getCapsuleIDfromHashMap(JSON.stringify(listChanges.msg3[0]).replace(/\"/g, ''));
			var trgMsgID = editor.getCapsuleIDfromHashMap(JSON.stringify(listChanges.msg3[1]).replace(/\"/g, ''));
		...
		if (listChanges.msg4 != null){
			//TODO: only one msg
			var srcMsgID = editor.getCapsuleIDfromHashMap(JSON.stringify(listChanges.msg4[0]).replace(/\"/g, ''));
			var trgMsgID = editor.getCapsuleIDfromHashMap(JSON.stringify(listChanges.msg4[1]).replace(/\"/g, ''));
		...
		console.log("itemName ============>"+JSON.stringify(itemName).replace(/\"/g, ''));
		...
		console.log("greenStateID============>"+greenStateID);
		...
		try
		{
           ...
           for (i =0; i< editor.lastCellsLength; i++){
			...
			//===========================================[Connectors]===================================================
			if (connID1.includes(",")){
				var connID1Split = connID1.split(",");
				for (var i = 0 ; i < connID1Split.length; i++){	
					cell = model.getCell(connID1Split[i]);
					if (cell != null)
					{
						editor.lastStyleGrayState = graph.getCellStyle(cell);
						...
						graph.removeCellOverlays(cell);
						editor.pushToLastIDs(connID1Split[i]);
					}
				}
			}else{
			...
			if (connID2.includes(",")){
				var connID1Split = connID2.split(",");
				for (var i = 0 ; i < connID2Split.length; i++){	
					cell = model.getCell(connID2Split[i]);
					if (cell != null)
					{
						editor.lastStyleGrayState = graph.getCellStyle(cell);
						graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, 'blue' , [cell]);
						...
						editor.pushTolastConnectorCells(cell); 
						graph.removeCellOverlays(cell);				
						editor.pushToLastIDs(connID2Split[i]);
					}
				}
			}else{
			...
			if (connID3.includes(",")){
				var connID3Split = connID3.split(",");
				for (var i = 0 ; i < connID3Split.length; i++){	
					cell = model.getCell(connID3Split[i]);
					if (cell != null)
					{
						editor.lastStyleGrayState = graph.getCellStyle(cell);
						graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, 'blue' , [cell]);
						...
						editor.pushTolastConnectorCells(cell); 
						graph.removeCellOverlays(cell);				
						editor.pushToLastIDs(connID3Split[i]);
					}
				}
			}else{
			...
			if (connID4.includes(",")){
				var connID4Split = connID4.split(",");
				for (var i = 0 ; i < connID4Split.length; i++){	
					cell = model.getCell(connID4Split[i]);
					if (cell != null)
					{
						editor.lastStyleGrayState = graph.getCellStyle(cell);
						graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, 'blue' , [cell]);
						...
						editor.pushTolastConnectorCells(cell); 
						graph.removeCellOverlays(cell);				
						editor.pushToLastIDs(connID4Split[i]);
					}
				}
			}else{
