...
//-------------------------------------------------------------- 	
	...
	Editor.prototype.pushToCapsuleActiveStateHashMap = function(capID, activeStateID)
	{
		...
		capID.toString();
		capsuleActiveStateHashMap.set(capID,activeStateID);
		...
		if (capsuleName != ""){
			for (var i = 0, keys = Object.keys(modelCapsuleAnalysis), j = keys.length; i < j; i++) {
				 ...
				 if (capsuleSplit[0].includes(capsuleName)){
					console.log(modelCapsuleAnalysis);
					//console.log('>>>>>> capsuleName om editor : ' + capsuleName);
				 	for (var k = 0 ; k<modelCapsuleAnalysis[keys[i]].length; k++){	
						...
						//console.log('---- itemValue om editor : ' + itemValue);
						//console.log('---- itemName om editor : ' + itemName);
						//console.log('---- itemID om editor : ' + itemID);
	...
	Editor.prototype.getIDConnectorFromHashMap = function(srcCapsuleID,trgCapsuleID)
	{
		...
		for (var i = 0, keys = Object.keys(modelConnectorAnalysis), j = keys.length; i < j; i++) {
			console.log('modelConnectorAnalysis[keys[i]] : ' + modelConnectorAnalysis[keys[i]]); 
			if ((modelConnectorAnalysis[keys[i]][0].ID !== 'undefined') && (modelConnectorAnalysis[keys[i]][1].ID !== 'undefined')){
				 var connID = keys[i];
				 var _srcCapsuleID = modelConnectorAnalysis[keys[i]][0].ID.toString();
				 var _trgCapsuleID = modelConnectorAnalysis[keys[i]][1].ID.toString();
				 
				 console.log(' _srcCapsuleID : ' + _srcCapsuleID + ' _trgCapsuleID : '+ _trgCapsuleID);
				 
				 if (_srcCapsuleID.includes(srcCapsuleID) && _trgCapsuleID.includes(trgCapsuleID)){
					 console.log('Found connID : ' + connID + ' _srcCapsuleID : ' + _srcCapsuleID + ' _trgCapsuleID : '+ _trgCapsuleID);
					 return connID
				 }
	...
	Editor.prototype.getCapsuleIDfromHashMap = function(capsuleName)
	{
	
		for (var i = 0, keys = Object.keys(modelCapsuleAnalysis), j = keys.length; i < j; i++) {
			 var capsuleSplit = keys[i].split(",");
			 if (capsuleSplit[0].includes(capsuleName)){
				console.log('capsuleName found : ' + capsuleName);
				console.log('CapsuleID found : ' + capsuleSplit[1]);
				return capsuleSplit[1];
			}
		}
	}
	...
	{		
		for (var i = 0, keys = Object.keys(modelCapsuleAnalysis), j = keys.length; i < j; i++) {
			 var capsuleSplit = keys[i].split(",");
			 	for (var k = 0 ; k<modelCapsuleAnalysis[keys[i]].length; k++){	
			 		var itemValue = modelCapsuleAnalysis[keys[i]][k][0].toString();
			 		var itemID = modelCapsuleAnalysis[keys[i]][k][1].toString();
					//console.log(itemValue);
					if (itemID.includes(greenStateID) == true){
						return capsuleSplit[1];
					}
				}
			 //}
