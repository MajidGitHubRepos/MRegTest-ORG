...
public class Handler extends Thread {
	...
	public void run() {
		try{
			...
			if (inMsg != null) {
				if (inMsg.contains("MAKE_LIST_EMPTY")) {
					System.out.println("==================================[ModelJsonServer.inMsgQueue.clear]============================");
					ModelJsonServer.inMsgQueue.clear();
					ModelJsonServer.emptyQueueForTesting = true;
					ModelJsonServer.openFileStream();
					//System.out.println("==================================[emptyQueueForTesting]============================"+ModelJsonServer.emptyQueueForTesting);
					
				}else if (ModelJsonServer.inMsgQueue.isEmpty()) {
					...
					Message msg = new Message( Integer.parseInt(inMsgJSONArrID.get(0).toString()), inMsgJSONArrID.get(1).toString(), inMsgJSONArrID.get(2).toString(), 
							inMsgJSONArrID.get(3).toString(), inMsgJSONTraceSizes.get(0).toString(), inMsgJSONTraceSizes.get(1).toString(), null, null);
					...
					if (inMsgJSON.has("infoMsg")) {
						JSONArray inMsgJSONInfoMsg = inMsgJSON.getJSONArray("infoMsg");
						if (inMsgJSONInfoMsg.length()>1) {
							msg.setInfoMsg_type(inMsgJSONInfoMsg.get(0).toString());
							msg.setInfoMsg_content(inMsgJSONInfoMsg.get(1).toString());
						}
					}
